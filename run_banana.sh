...
##TODO revert back to original proposal before run!
for ratio_s_var in 0.01 0.05 0.1 0.5 1. 5.
do
  CUDA_VISIBLE_DEVICES="" taskset -c 6-15 python experiments/ksd_all.py \
  --model=t-banana --dim=5 --nmodes=20 --nbanana=10 --T=100 --n=1000 --ratio_s_var=$ratio_s_var --t_std=0.1 --nrep=100 --mcmckernel=barker &
done

for dim in 2 5 10 15 20 25 30 35 40 45 50
do
  CUDA_VISIBLE_DEVICES="" taskset -c 16-30 python experiments/ksd_all.py \
  --model=t-banana --dim=$dim --nmodes=20 --nbanana=10 --T=100 --n=1000 --ratio_s_var=1. --t_std=0.1 --nrep=100 --mcmckernel=barker &
done
