...
def run_ksd_experiment(nrep, target, proposal_on, proposal_off, convolution, kernel, num_est, n, dim):
    """compute KSD and repeat for nrep times"""
    ksd = ConvolvedKSD(target=target, kernel=kernel, conv_kernel=convolution)
    
    ksd_df = pd.DataFrame(columns=["n", "ksd", "seed", "type", "dim"])
    for seed in range(nrep):
        ...
        ksd_df.loc[len(ksd_df)] = [n, ksd_val, seed, "off-target", dim]
        ...
        ksd_df.loc[len(ksd_df)] = [n, ksd_val, seed, "target", dim]
    return ksd_df
...
if __name__ == '__main__':
    ...
    for ind, var in enumerate(var_list):
        iterator = tqdm(dim_list)
        ksd_imq_ls = []
        ksd_rbf_ls = []
        for dim in iterator:
            iterator.set_description(f"Running with var = {var}, dim = {dim}")
            # target distribution
            ...
            # with IMQ
            imq = IMQ()
            ksd_imq_ls.append(run_ksd_experiment(nrep, target, proposal_on, proposal_off, convolution, imq, num_est, nsamples, dim))

            # with RBF
            rbf = RBF()
            ksd_rbf_ls.append(run_ksd_experiment(nrep, target, proposal_on, proposal_off, convolution, rbf, num_est, nsamples, dim))
        
        # collect results
        ksd_imq_df = pd.concat(ksd_imq_ls).reset_index(drop=True)
        ksd_rbf_df = pd.concat(ksd_rbf_ls).reset_index(drop=True)
        ...
        sns.lineplot(ax=axs[1], data=ksd_imq_df, x="dim", y="ksd", hue="type", style="type", markers=True)
        # _ = plt.ylim((0, None))
        ...
        sns.lineplot(ax=axs[2], data=ksd_rbf_df, x="dim", y="ksd", hue="type", style="type", markers=True)
        # _ = plt.ylim((0, None))
    ...
    # plt.tight_layout()
    fig.savefig(f"figs/mixture_gaussian_convolved_est_dims.png")