...
if __name__ == '__main__':
    ...
    # target distribution
    target, log_prob_fn = create_mixture_gaussian_kdim(dim=dim, k=k, delta=delta, ratio=ratio_target, return_logprob=True)
    ...
    if len(args.load) > 0 :
        try:
            test_imq_df = pd.read_csv(args.load + f"/mh_stepsize{std}_kdim{k}_ratio{ratio_target}_{ratio_sample}.csv")
    ...
    if test_imq_df is None:
        ...
        # save res
        test_imq_df.to_csv(f"res/bootstrap/mh_stepsize{std}_kdim{k}_ratio{ratio_target}_{ratio_sample}.csv", index=False)
    ...
    # plt.tight_layout()
    fig.savefig(f"figs/bootstrap/bootstrap_mh_kdim{k}_ratio{ratio_target}_{ratio_sample}.png")
