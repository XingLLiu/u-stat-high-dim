...
from src.ksd.models import create_mixture_gaussian_kdim
...
if __name__ == '__main__':
    ...
    for ind, delta in enumerate(delta_list):
        ...
        target_sample = proposal_on.sample(10000).numpy()
        target_sample_conv = target_sample + convolution_sample
        target_df1 = pd.DataFrame({"x1": target_sample[:, 0], "x2": target_sample[:, 1], "type": "target noiseless"})
        target_df2 = pd.DataFrame({"x1": target_sample_conv[:, 0], "x2": target_sample_conv[:, 1], "type": "target"})
        target_df = pd.concat([target_df1, target_df2], ignore_index=True)
        
        sns.kdeplot(ax=axs[0], data=target_df, x="x1", y="x2", hue="type", alpha=0.8, levels=10)
        axs[0].set_title("convolved target with median var over repetitions ({:.3g})".format(var.numpy()))
