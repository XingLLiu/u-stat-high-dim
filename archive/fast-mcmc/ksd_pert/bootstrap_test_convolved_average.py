            ...
            if "noiseless" in dist_name:
                ...
                for log_noise_std in log_noise_std_list:
                    ...
                    if p_val < smalles_p_val:
                        ...
                        smalles_p_val = p_val
                    # save var if it leads to a p-value <= alpha
                    if p_val <= alpha:
                        best_log_noise_std_ls.append(log_noise_std)

                # if 1 or more vars lead to p-value <= alpha, take the average for stability
                if len(best_log_noise_std_ls) > 0:
                    best_log_noise_std = np.mean(best_log_noise_std_ls)
...
if __name__ == '__main__':
    ...
    for ind, delta in enumerate(delta_list):
        ...
        if len(args.load) > 0 :
            try:
                test_imq_df = pd.read_csv(args.load + f"/average_delta{delta}.csv")
        ...
        # save res
        test_imq_df.to_csv(f"res/bootstrap/average_delta{delta}.csv", index=False)
    ...
    fig.savefig("figs/bootstrap/bootstrap_convolved_average.png")
