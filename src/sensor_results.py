...
def experiment(T, n, target_dist):
    ...
    for ram_scale in iterator:
        ...
        for i, seed in enumerate(range(REP)):
            ...
            res_ksdagg.append([ram_scale, ksdagg_rej, seed])
    ...
    res_df_ksdagg = pd.DataFrame(res_ksdagg, columns=["ram_scale", "ksdagg_rej", "seed"])
    res_df_ksdagg.to_csv(f"res/sensors/res_{model_name}_ksdagg.csv", index=False)
