...
import gpflow
from src.ksd.kernel import bandwidth
class KSDAuto:
  def __init__(
    self,
    target: tfp.distributions,
  ...
  def __call__(self, X: tf.Tensor, Y: tf.Tensor):
    ...
    # median heuristic
    if isinstance(self.k, gpflow.kernels.stationaries.SquaredExponential):
      # print(bandwidth(X, Y), tf.math.sqrt(2.))
      # self.k.lengthscales = tf.cast(bandwidth(X, Y) / tf.math.sqrt(2.), dtype=tf.float64)
      self.k.lengthscales = bandwidth(X, Y) / tf.math.sqrt(2.)
